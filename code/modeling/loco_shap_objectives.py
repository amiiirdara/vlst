...
objectives — pr_auc, f1, f2 — mirroring the FFS scoring loop in tabpfn.ipynb §10.
...
Example:
    ...
    python code/modeling/loco_shap_objectives.py --report-only --out data/result/loco_shap
"""
...
# ---------------------------------------------------------------------------
# Data & scoring (aligned with tabpfn.ipynb)
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "This matches the FFS holdout logic in `tabpfn.ipynb` but is optimistic if you iterate on test — "
...
def load_legacy_pr_auc_loco(out_dir: Path) -> pd.DataFrame | None:
    """Import existing pr_auc LOCO from data/figures if present."""
